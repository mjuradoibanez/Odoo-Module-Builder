              ...
              <View key={model.id || model.technicalName} style={{ marginBottom: 12, padding: 10, borderRadius: 8, backgroundColor: model.changeType !== 'unchanged' ? '#f7f7f7' : undefined, borderLeftWidth: 5, borderLeftColor: colorForChange(model.changeType) }}>
                ...
                {Array.isArray(model.fields) && model.fields.filter(f => f && typeof f === 'object').length > 0 && (
                  <View style={{ marginLeft: 10, marginTop: 4 }}>
                    <Text style={{ fontSize: 13, color: '#666', fontWeight: 'bold' }}>Campos:</Text>
                    {model.fields.filter(f => f && typeof f === 'object').map(field => (
                      <Text key={field.id || field.technicalName} style={{ color: colorForChange(field.changeType), fontSize: 14 }}>
                ...
                {Array.isArray(model.views) && model.views.filter(v => v && typeof v === 'object').length > 0 && (
                  <View style={{ marginLeft: 10, marginTop: 8 }}>
                    <Text style={{ fontSize: 13, color: '#666', fontWeight: 'bold' }}>Vistas:</Text>
                    {model.views.filter(v => v && typeof v === 'object').map(view => (
                      <Text key={view.id || view.name} style={{ color: colorForChange(view.changeType), fontSize: 14 }}>
                        - {view?.name ?? '(Sin nombre)'} <Text style={{ color: '#888' }}>({view?.type ?? '-'})</Text> {view?.changeType !== 'unchanged' && `[${view?.changeType === 'new' ? 'Nuevo' : view?.changeType === 'edit' ? 'Editado' : 'Eliminado'}]`}
                      </Text>
                    ))}
                  </View>
                )}
